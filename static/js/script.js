...
function updateNoiseChart() {
    ...
    if (updateCounter % updateInterval === 0) {
        const now = new Date();
        const timeString = now.getMinutes().toString().padStart(2, '0') + ':' +
...
document.getElementById('setupMicButton').addEventListener('click', setupMicrophone);
...
const expandSignal = [
    ...
    1.00000, 0.95106, 0.80902, 0.58779, 0.30902, 
    0.00000, -0.30902, -0.58779, -0.80902, -0.95106, 
    -1.00000, -0.95106, -0.80902, -0.58779, -0.30902,
]

const digitalSignal = [
    ...expandSignal, ...expandSignal, ...expandSignal, ...expandSignal,
    ...expandSignal, ...expandSignal, ...expandSignal, ...expandSignal,
    ...expandSignal, ...expandSignal, ...expandSignal, ...expandSignal,
    ...expandSignal, ...expandSignal, ...expandSignal, ...expandSignal,
    ...expandSignal, ...expandSignal, ...expandSignal, ...expandSignal,
...
function createAudioBuffer(digitalSignal) {
    const buffer = audioContext.createBuffer(1, digitalSignal.length, audioContext.sampleRate);
